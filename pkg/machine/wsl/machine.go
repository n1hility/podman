...
func (v *MachineVM) Start(name string, _ machine.StartOptions) error {
	...
	if err != nil {
		fmt.Println("API forwarding for Docker API clients is not available due to the following startup failures.")
		fmt.Printf("\t%s\n", err.Error())
...
func (v *MachineVM) Stop(name string, _ machine.StopOptions) error {
	...
	if err := stopWinProxy(v); err != nil {
		fmt.Printf("Could not stop API forwarding service (win-sshproxy.exe): %s\n", err.Error())
	}

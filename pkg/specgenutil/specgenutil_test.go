...
func TestWinPath(t *testing.T) {
	...
	f := func(vol string, mach string) (*specgen.SpecGenerator, error) {
		config := util.DefaultContainerConfig()
		oldEnable, oldType := config.Engine.MachineEnabled, config.Engine.MachineType
		config.Engine.MachineEnabled, config.Engine.MachineType = len(mach) > 0, mach
		sg := specgen.NewSpecGenerator("nothing", false)
		err := FillOutSpecGen(sg, &entities.ContainerCreateOptions{
			ImageVolume: "ignore",
			Volume:      []string{vol}}, []string{},
		)
		config.Engine.MachineEnabled, config.Engine.MachineType = oldEnable, oldType
		return sg, err
	}
